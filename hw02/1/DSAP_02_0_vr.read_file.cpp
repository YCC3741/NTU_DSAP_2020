...
using namespace std;
int main(){
    ifstream input_data("test.txt");
    int training_set, actual_set;

    if (input_data && !input_data.eof()){
        input_data >> training_set >> actual_set;
        input_data.ignore();
        ...
        while( !input_data.eof() && trian_run_num--){

            input_data >> gender[row_count];
            input_data.ignore();
            
            string temp_row;
            getline(input_data, temp_row);
        ...
        row_count = 0;
        while( !input_data.eof() && actual_run_num--){
            input_data >> actual_gender[row_count];
            input_data.ignore();
